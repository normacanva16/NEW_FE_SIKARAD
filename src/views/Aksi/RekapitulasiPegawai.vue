...
Vue.component("apexchart", VueApexCharts);
  ...
  try {
    ...
    const categories = response.data.series[0].data.map((item) => item[0]);

    // Match categories with optionKotama values and replace with corresponding text
    // const updatedCategories = categories.map((category) => {
    //   const matchedOption = this.optionKotama.find((option) => option.value === category);
    //   return matchedOption ? matchedOption.text : category;
    // });
